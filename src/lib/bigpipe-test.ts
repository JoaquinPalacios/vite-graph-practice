...
export class BigPipeTester {
  ...
  runAllTests(): BigPipeTestResult[] {
    ...
    console.log("[GRAPH] 🧪 Starting BigPipe Tests...");
  ...
  private printResults(): void {
    ...
    if (passed === total) {
      console.log("[GRAPH] 🎉 All BigPipe tests passed!");
    } else {
      console.log(
        "[GRAPH] ⚠️  Some BigPipe tests failed. Check the details above."
      );
    }
  }
  ...
  simulateBigPipeEvent(
    ...
    console.log(
      `[GRAPH] 🎭 Simulating BigPipe event: ${eventType}`,
      event.detail
    );
